...
print("Logistic Regression Estimated Betas (B0,B1):",logit1.intercept_[0],logit1.coef_[0])
...
x = np.linspace(np.min(heart[['Age']])-10,
np.max(heart[['Age']])+10,200)

x_plot = x.reshape(-1,1)

# be careful in pulling off only the correct column of the probability calculations: use `[:,1]`
yhat_class_knn20 = knn20.predict(x_plot)
yhat_prob_knn20 = knn20.predict_proba(x_plot)[:,1]

yhat_class_logit = logit1.predict(x_plot)
yhat_prob_logit = logit1.predict_proba(x_plot)[:,1]
