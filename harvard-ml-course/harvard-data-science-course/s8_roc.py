...
knn_fpr = np.array([get_fpr(y_test, y_pred_knn, t) for t in knn_thresholds])
# TPR for the kNN at each of its thresholds
knn_tpr = np.array([get_tpr(y_test, y_pred_knn, t) for t in knn_thresholds])

# TPR for the logistic model at each of its thresholds
logreg_tpr = np.array([get_tpr(y_test, y_pred_logreg, t) for t in logreg_thresholds])
# FPR for the logistic model at each of its thresholds
logreg_fpr = np.array([get_fpr(y_test, y_pred_logreg, t) for t in logreg_thresholds])
